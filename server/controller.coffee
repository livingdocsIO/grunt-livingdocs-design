...
livereloadPort = 35769
livereloadScript = "<script src=\"http://localhost:#{livereloadPort}/livereload.js\"></script>"
  ...
  listKickstarters: (req, res, next) ->
    ...
    files.forEach (kickstart) ->
      if kickstart.indexOf('.html') != -1
        list +=  "<li><a href=\"#{kickstartersUrl}/#{kickstart}\">#{kickstart}</a></li>"
